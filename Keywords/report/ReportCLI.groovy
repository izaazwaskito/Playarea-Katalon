...
public class ReportCLI {
	...
	def initReport(String tcId, String projectName, String docType) {
		reportData = [
			tcId: tcId, projectName: projectName, docType: docType,
	...
	def addSection(String sectionName) {
		...
		reportData.steps.add([level: 1, number: String.valueOf(nextNum), name: sectionName])
	}
	...
	def addStepCase(String stepName, String status, String notes, String imagePath) {
		...
		reportData.steps.add([
			level: 2, number: "${parentNum}.${childCount + 1}",
			name: stepName, status: status, notes: notes, imagePath: imagePath
		])
	}
	...
	def generatePDF(String outputPdfPath) {
		...
		if (process.exitValue() == 0) {
			...
			System.err.println("❌ Gagal: " + process.getErrorStream().getText())
		}
	}
}